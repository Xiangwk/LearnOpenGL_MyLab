...
float calcShadow(vec4 fpls, vec3 lightDir, vec3 normal);

void main()
{
    float dirShadow = calcShadow(FragPosDirLightSpace, dirLight.direction, Normal);
	...
	for(int i = 0; i < SpotLightNum; ++i)
	    result += calcSpotLight(torch[i], mapNormal, FragPos, viewPos);

	//add reflect map
    vec3 I = FragPos - viewPos;
	vec3 R = reflect(I, normalize(Normal));
	vec4 reflectColor = texture(skybox, R) * texture(material.texture_reflect1, TexCoord);

    fragColor = vec4(result, 1.0f) + reflectColor;
}
...
vec3 calcDirLight(DirectionalLight light, vec3 normal, vec3 viewPos, float shadow)
{
	...
	vec3 specular = light.specular * spec(ld, n, vd, material.shininess) * vec3(texture(material.texture_specular1, TexCoord));
...
vec3 calcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewPos)
{
    ...
    vec3 ld = normalize(lPos - fragPos);
	vec3 n = normalize(normal);
	vec3 vd = normalize(viewPos - FragPos);
